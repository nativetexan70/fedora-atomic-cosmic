...
1. **`Containerfile`** — pulls the base image (tag controlled via `FEDORA_VERSION` build
   arg, defaults to `44`), `COPY`s `system_files/` onto `/`, then `COPY`s and runs `build_files/build.sh`.
...
`quay.io/fedora-ostree-desktops/cosmic-atomic` does **not** publish a `:latest` tag — only
versioned tags (`:44`, `:43`, etc.). To update to a new Fedora release, bump `FEDORA_VERSION`
in both files above and update the version-specific output tag in the workflow's `tags:` lines
(e.g. `latest 45 ${{ steps.tags.outputs.date }}`).

The published image carries `latest`, the Fedora version number, and a `YYYYMMDD` date tag.
